...
try:
    ...
    raise ImportError(
        "Please install denoising_diffusion_pytorch with `pip install denoising_diffusion_pytorch`"
    )
...
def get_unet_params(model_name="unet_small", num_frames=4):
    ...
    if model_name == "unet_small":
        ...
        raise (
            f"Model name not found: {model_name}, choose between 'unet_small' or 'unet_big'"
        )
...
def get_uvit_params(model_name="uvit_small", num_frames=4):
    ...
    if model_name == "uvit_small":
        return dict(
            # dim=512,
            dim=256,  # reduce dims to fit in memory
            ff_mult=2,
            vit_depth=4,
            channels=4,
            patch_size=4,
        ...
        raise (
            f"Model name not found: {model_name}, choose between 'uvit_small' or 'uvit_big'"
        )
