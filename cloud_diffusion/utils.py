...
class NoisifyDataloader(DataLoader):
    ...
    def __init__(self, dataset, *args, noise_func=None, **kwargs):
        super().__init__(
            dataset, *args, collate_fn=noisify_collate(noise_func), **kwargs
        )
...
class MiniTrainer:
    ...
    def __init__(
        ...
        self.val_batch = next(iter(valid_dataloader))[0].to(
            device
        )  # grab a fixed batch to log predictions
    ...
    def prepare(self, config):
        ...
        self.scheduler = OneCycleLR(
            self.optimizer, max_lr=config.lr, total_steps=config.total_train_steps
        )
...
def rescale(x, min, max):
    "Rescales a tensor range from [min, max] to [0, 1]"
